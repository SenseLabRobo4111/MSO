...
\begin{table}[!htbp]
	...
	\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}Xrrrrrrr@{}}
		\toprule
		\multicolumn{8}{@{}l}{\textbf{a, Evaluated prediction models}} \\
		\rowcolor{TableHeader}
		\textbf{Method} & \shortstack{\textbf{Params}\\$\downarrow$} & \shortstack{\textbf{Operation entries}\\\textbf{(millions)} $\downarrow$} & \shortstack{\textbf{PSNR}\\$\uparrow$} & \shortstack{\textbf{SSIM}\\$\uparrow$} & \shortstack{\textbf{LPIPS}\\$\downarrow$} & \shortstack{\textbf{FID}\\$\downarrow$} & \shortstack{\textbf{KID}\\$\downarrow$} \\
		\midrule
		...
		\midrule
		\multicolumn{8}{@{}l}{\textbf{b, MSO component ablations}} \\
...
\begin{table}[!htbp]
	\centering
	\caption{\textbf{Map fusion comparison and component ablation.} \emph{a}, Predicted, observed and simulated map comparisons. Success rate (SR) denotes the fraction of pairs whose post-registration Dice exceeds 0.83. \emph{b}, Component ablation on matched inputs from the predicted-map split. Legacy RMSE, SSIM and Dice are descriptive post-warp summaries; they do not directly measure relative-pose error. The archived source does not preserve the raster scaling used for RMSE, so its unitless values support within-table description only. All ablation values are point estimates without uncertainty intervals or inferential ranking.}
	\label{tab:fusion-results}
	\MSOTableSetup
	\begin{tabularx}{\textwidth}{@{}l>{\raggedright\arraybackslash}Xrrrr@{}}
		\toprule
		\multicolumn{6}{@{}l}{\textbf{a, Map and registration-algorithm comparison}} \\
		\rowcolor{TableHeader}
	...
	\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lcccc@{}}
		\toprule
		\multicolumn{5}{@{}l}{\textbf{b, Predicted-map component ablation}} \\
		\rowcolor{TableHeader}
		\textbf{Variant} & \textbf{SR} $\uparrow$ & \textbf{Dice} $\uparrow$ & \textbf{Legacy RMSE} $\downarrow$ & \textbf{SSIM} $\uparrow$ \\
		\midrule
...
We separately inject a fixed 1.0\,m and 15$^{\circ}$ error into 150 positive proposals. The ground-truth-free observed-support gate rejects all 150 before the harness's simulated atomic OR-merge; the harness target-mask hash remains unchanged and the changed-cell count is zero for every rejection (Fig.~\ref{fig:transform-audit}d; Supplementary Table~5). Feature matching is bypassed in this test. The result characterises the offline harness at the injected error level only and does not establish deployed commit behaviour, rollback or recovery.
...
Panel~b of Table~\ref{tab:fusion-results} reports the fusion component ablation. Disabling ambiguity-heuristic weighting changes Dice from $0.588$ to $0.534$. Removing the $0.5{\times}$ and $0.25{\times}$ feature pools changes legacy RMSE from $0.797$ to $0.842$ and SSIM from $0.944$ to $0.934$. Replacing staged RANSAC with one relaxed pass changes the strict success rate from $0.125$ to $0.106$. No uncertainty interval or hypothesis test is available, so these point differences are not interpreted as statistically established rankings.
