...
It is not the private/deployed MSO code and must not be used to relabel the
...
def _candidate_matches(source: FeaturePool, target: FeaturePool) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for family in (0, 1, 2):
        ...
        if len(knn_survivors) < 8:
            direct = matcher.match(source.descriptors[s_idx], target.descriptors[t_idx])
            direct.sort(key=lambda item: item.distance)
